...
function initFormsMailChimp () {
    ...
    $.validator.addMethod('mc_birthday', function (date, element, grouping_class) {
        ...
        if ($fields.filter(':filled').length === 0 && this.optional(element)) {
            ...
            let testDate = new Date(1970, dateArray.month, dateArray.day);
    ...
    $.validator.addMethod('mc_date', function (date, element, grouping_class) {
        ...
        if ($fields.filter(':filled').length === 0 && this.optional(element)) {
            ...
            let testDate = new Date(dateArray.year, dateArray.month, dateArray.day);
    ...
    $.validator.addMethod('mc_phone', function (phone_number, element, grouping_class) {
        let isValid = false;
        let $fields = $('input:filled:not(:hidden)', $(element).closest(grouping_class));
        ...
        let validator = $form.validate({
            ...
            groups: function () {
                ...
                $form.find('.input-group').each(function () {
                    let inputs = $(this).find('input:text:not(:hidden)');	// TODO: What about non-text inputs like number?
                    if (inputs.length > 1) {
                        let mergeName = inputs.first().attr('name');
                        let fieldNames = $.map(inputs, function (f) {
                            return f.name;
                        });
                        ...
                        if (resp.result === 'success') {
                            ...
                            try {
                                ...
                                if (index === -1) {
                                    ...
                                    let fieldName = $form.find('input[name]:eq(' + index + ')').attr('name'); // Make sure this exists
                                    let data = {};
                                    data[fieldName] = msg;
